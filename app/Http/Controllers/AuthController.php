...

{
  ...
  public function log(Request $request, messenger $messenger)
  {
    ...
    if ($resp->status === true) {
      ...
      return redirect('/login')->with('message', 'There was an error while trying to login ')->with("type", "danger");
    }
  }
  ...
  public function register(Request $request, Messenger $messenger)
  {
    ...
    if ($response->status === true) {
      ...
      return redirect('/login')->with('message', 'You have successfully registered, kindly login ')->with("type", "success");
    } else if ($response->status === false) {

      return redirect('/register')->with('message', 'This eamil has already been taken ')->with("type", "danger");
    }
  }
